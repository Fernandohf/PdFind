...
def export_pdf(playground, default_dpi, savefile, use_tqdm=False):
    ...
    dpi = default_dpi
    for image in images if not use_tqdm else tqdm(images):
        im = Image.open(image)
        w, h = im.size
        try:
            dpi = im.info['dpi'][0]
...
def add_text_layer(pdf, image, height, dpi):
    ...
    for line in hocr.xpath('//*[@class="ocr_line"]'):
        ...
        for word in line.xpath('.//*[@class="ocrx_word"]'):
            ...
            b = polyval(baseline, (box[0] + box[2]) /
                        2 - linebox[0]) + linebox[3]
            text = pdf.beginText()
